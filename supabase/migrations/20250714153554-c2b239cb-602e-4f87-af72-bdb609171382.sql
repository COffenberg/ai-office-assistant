...
BEGIN
  ...
  SELECT 
    ...
    (
      CASE 
        ...
        ELSE 0.4
      END
    )::FLOAT as qa_relevance_score,
    (
      CASE 
        WHEN user_context ? qa.category THEN 0.2
        ELSE 0.0
      END
    )::FLOAT as qa_context_match
  ...
  -- Enhanced document search with AI summary and better relevance scoring
  SELECT 
    ...
    (
      CASE 
        ...
        -- Additional scoring for equipment, installation, package keywords
  ...
  ORDER BY (qa_relevance_score + qa_context_match + doc_relevance_score + doc_context_match) DESC
  LIMIT limit_results;
END;
$$;