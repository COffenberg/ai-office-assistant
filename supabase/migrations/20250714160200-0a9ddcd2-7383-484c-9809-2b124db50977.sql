-- Fix the ai_enhanced_search function ORDER BY clause and enhance semantic matching
...
BEGIN
  RETURN QUERY
  -- Enhanced Q&A search with better semantic matching
  SELECT 
  ...
  WHERE qa.is_active = true
    AND (
      ...
      -- Enhanced semantic matching patterns
      ...
      (search_query ILIKE '%customer%before%' AND (qa.question ILIKE '%customer%' OR qa.answer ILIKE '%customer%'))
    )
  ...
  SELECT 
    ...
    (
      CASE 
        ...
        -- Enhanced matching for customer communication questions
        WHEN (search_query ILIKE '%call customer%' OR search_query ILIKE '%customer%before%') 
             AND dc.content ILIKE '%customer%' THEN 0.9
        WHEN (search_query ILIKE '%number%call%' OR search_query ILIKE '%call%number%') 
             AND (dc.content ILIKE '%phone%' OR dc.content ILIKE '%support%') THEN 0.9
  ...
  WHERE d.processing_status IN ('processed', 'uploaded')
    AND (
      ...
      -- Enhanced matching for customer communication
      (search_query ILIKE '%call customer%' AND dc.content ILIKE '%customer%') OR
      (search_query ILIKE '%customer%before%' AND dc.content ILIKE '%customer%') OR
      (search_query ILIKE '%number%call%' AND (dc.content ILIKE '%phone%' OR dc.content ILIKE '%support%')) OR
      (search_query ILIKE '%call%number%' AND (dc.content ILIKE '%phone%' OR dc.content ILIKE '%support%')) OR
  ...
  -- Use proper column aliases for ORDER BY to avoid ambiguity
